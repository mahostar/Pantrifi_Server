...
def filter_users_with_sheets_or_csv():
    ...
    # Filter users who have at least one Google Sheet, CSV file, or menu file
    for user in data.get('subscribed_users', []):
        ...
        # Check if user has Google Sheets OR CSV files OR menu files
        if len(google_sheets) > 0 or len(csv_files) > 0 or len(menu_files) > 0:
            user_data = {
            ...
            # Only add user if they have at least one valid sheet URL, CSV URL, or menu URL
            if user_data['google_sheets_urls'] or user_data['csv_file_urls'] or user_data['menu_file_urls']:
